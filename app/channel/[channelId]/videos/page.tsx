// app/channel/[channelId]/videos/page.tsx
"use client"
        ...
        <TabsContent value="videos" className="pt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {channel.videos.length === 0 ? (
              <p className="col-span-full text-center text-muted-foreground py-10">No Puns uploaded yet.</p>
            ) : (
              channel.videos.map((video) => <VideoCard key={video.id} video={video} channel={channel} />)
            )}
          </div>
        </TabsContent>
      </Tabs>
